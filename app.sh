...
function refresh_token {
    ...
    [ $NEW_ACCESS_TOKEN ] && ACCESS_TOKEN=$NEW_ACCESS_TOKEN && sed -i "s/^ACCESS_TOKEN.*/ACCESS_TOKEN=$NEW_ACCESS_TOKEN/" $SCRIPT_NAME &&
    echo "Access token has been refreshed."
}

function get_list_one_page {
    local OUTPUT_ARRAY=`curl "${SRV_ENDPOINT}${MEDIA_OBJS_LIST}?key=$API_KEY&pageSize=10&pageToken=$NEXT_PAGE_TOKEN" \
    --header "Authorization: Bearer $ACCESS_TOKEN" --header 'Accept: application/json' --compressed`
    
    #echo "$OUTPUT_ARRAY"; exit
    if echo "$OUTPUT_ARRAY" | grep 'UNAUTHENTICATED' > /dev/null; then
        refresh_token &&
        return 0
    fi
    PAGE=(`echo "$OUTPUT_ARRAY" \
